...
void ini_spi()
{
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE); // zegar dla portu GPIO z ktÃ³rego wykorzystane zostanÂ¹ piny do SPI (MOSI, MISO, SCK)
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOE, ENABLE); // zegar dla portu GPIO z ktÃ³rego wykorzystany zostanie pin do SPI (CS)
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_SPI2, ENABLE); // zegar dla moduÂ³u SPI2
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD, ENABLE);

	GPIO_InitTypeDef GPIO_InitStructure;


	//inicjalizacja pinÃ³w wykorzystywanych do SPI
	...
	// deselecy â€“ ustawienie pinu w stan wysoki
...
void otwarcie_pliku(void)
{
	...
	if(znak == '1')	dioda_sygnal(1,3);
	else
	if(znak == '2')	dioda_sygnal(2,3);
	else
	if(znak == '3')	dioda_sygnal(3,3);
	else
	if(znak == '4')	dioda_sygnal(4,3);
}
...
void zapisz_glosnosc(int poziom)
//niedokoñczona funkcja
{

	WORD ile_bajtow;
	fresult = f_open (&usta, "glos.ust", FA_CREATE_ALWAYS | FA_WRITE);

	fresult = f_write(&usta, poziom, 2, &ile_bajtow);
	dioda_sygnal(3,1);
	fresult = f_close (&plik2);
	if (fresult == FR_OK)
		return 1;
